...
export default function CaseWorkspace({
  ...
  const selectedDocument = initialCase.documents.find(d => d.id === selectedDocId) || initialCase.documents[0];
        ...
        <div className="bg-white p-6 rounded-2xl border border-gray-200 shadow-sm transition-all duration-300">
          <div className="flex justify-between items-start">
            <div>
              ...
              <h1 className="text-2xl font-bold text-gray-900 mt-3">{initialCase.title}</h1>
              <div className="flex items-center gap-3 mt-2 text-sm text-gray-500">
                <span className="capitalize">Type: {initialCase.caseType?.replace("_", " ") || "Eviction Notice"}</span>
                <span>&bull;</span>
            ...
            {initialCase.latestUrgency && (
              <span className={`px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wider
                ${initialCase.latestUrgency === 'high' ? 'bg-red-50 text-red-700 border border-red-200' : 
                  initialCase.latestUrgency === 'medium' ? 'bg-yellow-50 text-yellow-700 border border-yellow-200' : 
        ...
        {initialCase.documents.length >= 1 && (
          <div className="bg-white p-5 rounded-2xl border border-gray-200 shadow-sm space-y-3">
            ...
            <div className="relative flex items-center justify-start gap-4 overflow-x-auto py-2">
              ...
              {initialCase.documents.slice().reverse().map((doc, index) => {
            ...
            {analysis.extraction && (
              <div className="bg-white p-6 rounded-2xl border border-gray-200 shadow-sm">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 text-sm">
                  ...
                  {(analysis.extraction.obligations?.length > 0 || analysis.extraction.financial_amount || analysis.extraction.penalties?.length > 0) && (
                    <div className="col-span-1 md:col-span-2 bg-gray-50 p-4 rounded-xl border border-gray-100 space-y-3">
                      <div>
                        ...
                        {analysis.extraction.obligations && analysis.extraction.obligations.length > 0 ? (
                          ...
                          <p className="text-gray-500 italic text-xs">None specified</p>
                        )}
                      </div>
                      
                      {analysis.extraction.penalties && analysis.extraction.penalties.length > 0 && (
                        <div>
                          <span className="text-gray-400 font-medium block text-xs uppercase tracking-wider mb-1">Monetary / Penalty Signals</span>
                          <p className="font-bold text-gray-900 text-sm">{analysis.extraction.penalties.join(", ")}</p>
                        </div>
                      )}
                    </div>
                  )}
                  ...
                  {analysis.extraction.legal_sections?.length > 0 && (
                    <div className="col-span-1 md:col-span-2">
                      <span className="text-gray-400 font-medium block text-xs uppercase tracking-wider mb-2">Legal Sections Cited</span>
                      <div className="flex flex-wrap gap-1.5">
                        {analysis.extraction.legal_sections.map((s: string, i: number) => (
                          <span key={i} className="px-3 py-1 bg-gray-100 border border-gray-200/50 text-gray-700 rounded-lg text-xs font-semibold">{s}</span>
                        ))}
                      </div>
                    </div>
                  )}
